...
class PlayController extends \Phalcon\Mvc\Controller
{
    ...
    public function playAction()
    {
        ...
        if(empty($items)) return $this->response->setJsonContent(['status' => 'error', 'msg' => '<div><div><strong>Ошибка</strong><br>В данный момент на ботах нет вещей для выбранного кейса. Попробуйте позже.</div></div>']);
        shuffle($items);
        $item = $items[0];
        $game = new Games();
